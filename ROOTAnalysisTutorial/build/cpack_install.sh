...
# Install the project:
DESTDIR=${DESTDIR}/usr/UserAnalysis/0.0.1 \
    make -k install/fast

# Remove the .dbg files from being packaged, in case we are in RelWithDebInfo
# build mode:
rm -f ${DESTDIR}/usr/UserAnalysis/0.0.1/InstallArea/x86_64-centos7-gcc8-opt/bin/*.dbg
rm -f ${DESTDIR}/usr/UserAnalysis/0.0.1/InstallArea/x86_64-centos7-gcc8-opt/lib/*.dbg
